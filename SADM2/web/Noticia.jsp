...
<!DOCTYPE html>
<html>
    <%Noticia noticiax = new Noticia(); 
    String fechaNueva =new String();
    
                                    //System.out.println("ID INICIO:..::: " + request.getParameter("id_html"));
            Consultas consultas = new Consultas();
                        if(null!=request.getParameter("id_html")){
                                    Noticia noti = consultas.obtNoticiabyId(request.getParameter("id_html"));
                                    noticiax = noti;
                                    
                                    String fechaRecibida = noti.getNoticia_fecha();
                                    ...
                                    fechaNueva = new SimpleDateFormat("MMMM dd, yyyy").format(fechaConcreta);
                                    fechaNueva= fechaNueva.substring(0, 1).toUpperCase() + fechaNueva.substring(1); 
                                    //System.out.println("fechaNueva ::::  " + fechaNueva);
                                    //System.out.println("CONTENT ::::  " + noti.getId_Noticia());
                                      String content = new String();
                                      content = noti.getCont_notice();
                                       
                        }else{ 
                   
                                    System.out.println("CONTENT NULL_ID ::::  " + request.getParameter("id_html"));
                                      
                         response.sendRedirect("Prensa.jsp");
                        }
                                   
            %>
    <head>
        <meta charset="utf-8"/>
        <title><%=noticiax.getTitle_notice_noticia()%></title>
    ...
    <form id="FormContentNoticia" name="FormContentNoticia">
        <br><br><br><br>
        <div  id="pren_img_noticia_edit" name="pren_img_noticia_edit" class="herosectionnoticias small" style="margin-left: auto;margin-right: auto;width:100%;background-image:url(&quot;<%=noticiax.getH3_img_noticia()%>&quot;);"></div>
          
        <div class="contenidonoticias bottom-padding">
            <div class="content-wrapper narrow w-container">
                <h1 data-ix="move-up-on-load-2" class="heading-4 bottom-border"><%=noticiax.getTitle_notice_noticia()%></h1>
                <h2><%=noticiax.getEncab_notice() %></h2>
                <p data-ix="move-up-on-load-3"><%=noticiax.getCont_notice()%></p>
                <div data-ix="move-up-on-load-4" class="box-details">
                    ...
                    <div class="inline-text"> <input type="text" readonly value="<%=fechaNueva%>" style="border: 0;"  >.</div></div>
                <div class="top-border-copy w-clearfix">
                    <a data-ix="move-up-on-load-3" href="/SADM/Prensa.jsp" class="small-link-2 top-border">Regresar a Noticias</a>
                </div>
            </div>
        </div>
